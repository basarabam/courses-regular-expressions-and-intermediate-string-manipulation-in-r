...
Input vector, usually character vector, as first argument
Pattern can be a string or regular expression, as second argument
Replacement a character vector of replacements as third argument. Vector of replacements should be either length one, or the same length as string or pattern, otherwise strange behavior may occur.
Replacement argument may take named vector or function for performing multiple replacements.
At new job you get the task to remove all vowels from the name of different sports. It is necessary for some letter guessing  game in the quiz, your company produces. If you have the list of sports, that is not a problem at all. You are going to do the task for 2 minutes, with stringr package! 
You have a str character vector, containing names of some sports, for testing purpose
At first you are going to test the str_replace function, with regular expression for vowels, and replace vowels with underscores.
First, you have to define a character class of vowels, in a form of regular expression, as the pattern variable. In this case, you are going to use square brackets, to define characters which you want to include in your search pattern.
Then you have to define a desired replacement variable, in this case it would be the underscore.
...
You can use str_replace_all() function, with the same arguments. This function will replace all vowels in the character vector.
Suddenly, your task is changed and you have to remove consonants from the sting. No problem you can use negation in regular expression syntax, and do it with just one key stroke.
...
`@script`
In the str_replace functions, you can perform multiple replacements by supplying named vector like in the example.
So don't celebrate with friends too much, your new replacing knowledge.
...
`@script`
In the following code example, the order of the second and third word is flipped, with backreference what would be discussed in the next chapter of these course.
This is example of efective use of regular expressions.
